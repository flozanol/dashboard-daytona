...
export default function ForecastDashboard() {
  ...
  const ventasChartData = [
    ...
    {
      mes: headers.mesActualRealLabel || 'Real',
      nuevos: ventasNuevos?.mesActualReal || 0,
      seminuevos: ventasSemi?.mesActualReal || 0,
    },
    {
      mes: `${mesActualLabel} Forecast`,
      nuevos: ventasNuevos?.forecastIA || 0,
      seminuevos: ventasSemi?.forecastIA || 0,
    },
  ];
              ...
              <tbody>
                {data.filter(row => row.metric).map((row, idx) => (
                  ...
                  >
                    ...
                    {(headers.historical || []).map(mes => (
                      <td key={mes} className="py-3 px-4 text-right text-gray-600" suppressHydrationWarning>
                        {(row.historical[mes] || 0).toFixed(0)}
                      </td>
                    ))}
                    <td className="py-3 px-4 text-right text-gray-600" suppressHydrationWarning>
                      {row.mesActualReal.toFixed(0)}
                    </td>
                    <td className="py-3 px-4 text-right font-bold text-red-600 text-base" suppressHydrationWarning>
                      {row.forecastIA.toFixed(0)}
                    </td>
                  </tr>
                ))}
              </tbody>
